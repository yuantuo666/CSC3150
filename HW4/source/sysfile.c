...
sys_open()
{
  ...
  int symlink_retry = 0;
  while (ip->type == T_SYMLINK && (omode & O_NOFOLLOW) == 0 && symlink_retry < 10) {
    // change the ip to the target of the symlink
    char target[MAXPATH];
    // read the target string into target
...
sys_symlink(void) {
  ...
  // write the target string into inode's data block
